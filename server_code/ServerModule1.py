...
@anvil.server.callable
def lagre_deltager_i_trekning(mandag):
    """
    Legger den innloggede brukeren til i trekningslisten for den spesifikke uken.

    Args:
        mandag (date): Datoen for mandagen i uken trekningen gjelder for.

    Returns:
        str: En melding som indikerer resultatet (suksess eller allerede registrert).

    Raises:
        anvil.server.PermissionDenied: Hvis ingen bruker er logget inn.
        ValueError: Hvis ingen korresponderende trekning finnes for den gitte mandagen.
        Exception: Ved andre feil under databaseoperasjoner.
    """
    # 1. Hent innlogget bruker
    user = anvil.users.get_user()
    if not user:
        raise anvil.server.PermissionDenied("Du må være logget inn for å melde deg på trekningen.")

    # 2. Valider input (enkel type sjekk)
    if not isinstance(mandag, date):
        raise TypeError("Parameter 'mandag' må være et date objekt.")

    print(f"Forsøker å melde på bruker {user['email']} for trekning uke {mandag.strftime('%Y-%m-%d')}")

    try:
        # 3. Finn riktig 'trekning'-rad basert på mandagen
        # Bruker get() siden vi forventer maks én trekning per mandag
        trekning_record = app_tables.trekninger.get(uke_mandag=mandag)

        if not trekning_record:
            print(f"FEIL: Fant ingen trekning for mandag {mandag}")
            raise ValueError(f"Systemfeil: Fant ingen trekning definert for uken som starter {mandag.strftime('%Y-%m-%d')}.")

        # 4. Sjekk om brukeren allerede er registrert for denne spesifikke trekningen
        existing_entry = app_tables.trekningsliste.get(user=user, trekning=trekning_record)

        if existing_entry:
            print(f"Bruker {user['email']} er allerede påmeldt trekning {trekning_record['nummer']} ({mandag})")
            return f"Du er allerede påmeldt trekningen for denne uken."
        else:
            # 5. Bruker er ikke registrert, legg til ny rad
            print(f"Legger til bruker {user['email']} i trekningsliste for trekning {trekning_record['nummer']} ({mandag})")
            app_tables.trekningsliste.add_row(
                user=user,
                trekning=trekning_record
            )
            return f"Du er nå påmeldt trekningen for uken som starter {mandag.strftime('%Y-%m-%d')}!"

    except anvil.tables.TableError as e:
        print(f"Databasefeil ved påmelding til trekning for {mandag}: {e}")
        raise Exception(f"En databasefeil oppstod under påmelding: {e}")
    except Exception as e:
        print(f"Uventet feil ved påmelding til trekning for {mandag}: {e}")
        raise Exception(f"En uventet feil oppstod: {e}")