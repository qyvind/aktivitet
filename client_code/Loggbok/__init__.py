...
class Loggbok(LoggbokTemplate):
    def __init__(self, **properties):
        ...
        self.man_column_panel.role = ""  # Fjern eventuell rolle
        self.man_column_panel.style = "background-color: hotpink; border: 3px dashed red;"

        # self.man_column_panel.classes = ["dag-kort"]
    ...
    def oppdater_dag(self, knapp, panel, label, aktivitet_data, dato, today_date, ikon_komponent, column_panel):
        ...
        # 💡 Fjern blokkering fra gamle roller og bruk CSS-klasser via 'role'
        panel.role = f"dag-poeng-{poeng}"
        if fremtid:
            panel.role += " fremtidig-dag"
    
        # 👇 Ekstra metadata hvis du vil bruke .tag senere
